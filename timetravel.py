...
train = pd.read_csv('C:\\Tools\\Python\\Programs\\Alex---CSC245\\Time Travel Project\\input\\act_train.csv', parse_dates=['date'])
test = pd.read_csv('C:\\Tools\\Python\\Programs\\Alex---CSC245\\Time Travel Project\\input\\act_test.csv', parse_dates=['date'])
ppl = pd.read_csv('C:\\Tools\\Python\\Programs\\Alex---CSC245\\Time Travel Project\\input\\people.csv', parse_dates=['date'])

df_train = pd.merge(train, ppl, on='people_id')
df_test = pd.merge(test, ppl, on='people_id')
del train, test, ppl

##First just so we know what we're dealing with, 
...
# So we can see that all the dates are a few years in the future, 
# all the way until 2023! Although we now though that this is because the 
# data was anonymised, so we can essentially treat these as if they 
# were the last few years instead.

# We can also see that date_x is on the order of 1 year, while date_y is 3 
# times longer, even though they both end on the same day (the date before 
# they stopped collecting the dataset perhaps?)
...
# We need to split it into multiple graphs since the time-scale is 
# too long to show well on one graph
i = int(len(date_y) / 3)
